...
export class PaymentController {
  ...
  async getAllPayments(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await this.paymentService.getAllPayments(userId, {
  ...
  async getPaymentStatistics(req: Request, res: Response): Promise<void> {
    try {
      ...
      const statistics = await this.paymentService.getPaymentStatistics(userId);
      res.status(200).json(statistics);
    } catch (error) {
      this.handleError(res, error, 'Failed to fetch payment statistics');
    }
  }

  async getPaymentMethodsDistribution(req: Request, res: Response): Promise<void> {
    try {
      const userId = req.user?.id;
      if (!userId) {
        res.status(400).json({ message: 'User ID is required' });
      ...
      const distribution = await this.paymentService.getPaymentMethodsDistribution(userId);
      res.status(200).json(distribution);
    } catch (error) {
      this.handleError(res, error, 'Failed to fetch payment methods distribution');
    }
  }
  ...
  async getRevenueByMonth(req: Request, res: Response): Promise<void> {
    try {
      ...
      const monthlyRevenue = await this.paymentService.getRevenueByMonth(userId);
      res.status(200).json(monthlyRevenue);
    } catch (error) {
      this.handleError(res, error, 'Failed to fetch revenue by month');
    }
  }
}